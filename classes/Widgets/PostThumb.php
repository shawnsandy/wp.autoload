 ...
 * @author studio
 */
class Widgets_PostThumb extends WP_Widget {
    ...
    public function __construct() {
         ...
         */
        $widget_id = 'post-thumb';
        $widget_name = 'Post Thumb';
        $widget_description = 'Displays a single article from post, page, custom_types choose - post_types, thumbnail size, hide title and description...';
        $width = 300;
    ...
    function form($instance) {
        ...
        $defaults = array('title' => __('Recent Post', 'recent-thumbs-widget'), 'page' => 'post', 'post_id' => '', 'desc_words' => 25, 'desc' => 'ON', 'thumb_size' => 'recent-thumb');
        $instance = wp_parse_args((array) $instance, $defaults);

        $query = $this->query($instance['page']);
        ...
        <p>
            <label for="<?php echo $this->get_field_id('post_id'); ?>"><?php _e('Post:', 'recent-thumbs-widget'); ?></label>

            <select id="<?php echo $this->get_field_id('post_id'); ?>" name="<?php echo $this->get_field_name('post_id'); ?>"
                   value="<?php echo $instance['post_id']; ?>" style="width:90%;">
                <option></option>
                <?php foreach ($query as $data): ?>
                <option value="<?php echo $data->ID ?>" <?php selected($data->ID, $instance['post_id']); ?>><?php echo $data->post_title ?></option>
                <?php endforeach; ?>
            </select>
        </p>
        <!-- Your Name: Text Input -->
        <p>
            ...
            <select id="<?php echo $this->get_field_id('thumb_size'); ?>" name="<?php echo $this->get_field_name('thumb_size'); ?>" style="width:90%;" >
                ...
                <option value="<?php echo $size ?>" <?php selected($size, $instance['thumb_size']); ?>><?php echo $size ?></option>
                <?php endforeach; ?>
            </select>
    ...
    public function query($post_type) {
